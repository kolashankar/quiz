    ...
    <div className="p-4 md:p-6 lg:p-8 max-w-7xl mx-auto">
        ...
        <Card className="mb-6">
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
            ...
            <div className="text-center">
              ...
              <div className="text-sm text-gray-600 mt-1">Strong Topics</div>
            </div>
          </div>
        </Card>
      )}
      ...
      <Card className="mb-6">
        ...
        <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
          <Link
            href="/dashboard/quiz"
            className="flex flex-col items-center p-4 rounded-lg border-2 border-gray-200 hover:border-primary hover:bg-primary/5 transition"
          >
            <BookOpenIcon className="w-10 h-10 text-primary mb-2" />
            <span className="text-sm font-medium text-gray-900 text-center">Take Quiz</span>
          </Link>
          <Link
            href="/dashboard/bookmarks"
            className="flex flex-col items-center p-4 rounded-lg border-2 border-gray-200 hover:border-warning hover:bg-warning/5 transition"
          >
            <BookmarkIcon className="w-10 h-10 text-warning mb-2" />
            <span className="text-sm font-medium text-gray-900 text-center">Bookmarks</span>
          </Link>
          <Link
            href="/dashboard/leaderboard"
            className="flex flex-col items-center p-4 rounded-lg border-2 border-gray-200 hover:border-success hover:bg-success/5 transition"
          >
            <TrophyIcon className="w-10 h-10 text-success mb-2" />
            <span className="text-sm font-medium text-gray-900 text-center">Leaderboard</span>
          </Link>
          <Link
            href="/dashboard/analytics"
            className="flex flex-col items-center p-4 rounded-lg border-2 border-gray-200 hover:border-purple-600 hover:bg-purple-50 transition"
          >
            <ChartBarIcon className="w-10 h-10 text-purple-600 mb-2" />
      ...
      {recommendations && recommendations.recommended_topics && (
        <Card className="mb-6">
          ...
          {recommendations.message && (
            <p className="text-gray-600 italic mb-4">{recommendations.message}</p>
          )}
          <div className="space-y-3">
            {recommendations.recommended_topics.slice(0, 3).map((topic: any, index: number) => (
              <div
                key={index}
                className="p-4 rounded-lg bg-gradient-to-r from-warning/10 to-warning/5 border border-warning/20"
              >
