...
export default function CustomDrawer({ visible, onClose }: DrawerProps) {
  ...
  // Fetch exams when switcher is opened
  useEffect(() => {
  ...
  const handleExamSwitch = async (examId: string) => {
    ...
    try {
      ...
      // Refresh user data to get updated exam info
      const updatedUser = await authService.getCurrentUser();
      // You might want to update the auth context here
      setShowExamSwitcher(false);
      onClose();
      // Show success message or refresh the UI
    } catch (error) {
      console.error('Error switching exam:', error);
      alert('Failed to switch exam. Please try again.');
    } finally {
  ...
  return (
    <>
          ...
          <Pressable style={[styles.drawer, { backgroundColor: colors.background }]} onPress={(e) => e.stopPropagation()}>
            ...
            <View style={[styles.header, { backgroundColor: colors.primary }]}>
              <View style={styles.headerContent}>
                {/* Profile Avatar - Show uploaded image or default icon */}
                {user?.avatar ? (
                  <Image 
                ...
                <View style={styles.userInfo}>
                  ...
                  {/* Show selected exam instead of "Student" */}
      ...
      {/* Exam Switcher Modal with Scrollable List */}
      <Modal
      ...
      >
        <Pressable
          ...
          onPress={() => !switchingExam && setShowExamSwitcher(false)}
        >
                  ...
                  exams.map((exam) => {
                    ...
                    return (
                      <TouchableOpacity
                        ...
                        style={[
                          styles.examItem,
                          isSelected && styles.selectedExamItem,
                      ...
                      >
                        <View style={styles.examContent}>
                          <Ionicons
                            name={isSelected ? "checkmark-circle" : "school"}
                            size={32}
                            color={isSelected ? "#34C759" : "#007AFF"}
                          />
                          <View style={styles.examTextContainer}>
                            <Text style={[styles.examName, { color: colors.text }]} numberOfLines={2}>
                              {exam.name}
                            </Text>
                            {exam.description && (
                              <Text style={[styles.examDescription, { color: colors.textSecondary }]} numberOfLines={1}>
                                {exam.description}
                              </Text>
                            )}
                          </View>
